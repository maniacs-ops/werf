...
> **REVIEW**
>
> Введение -> Обзор задачи и Требования
> 
> Предлагаю придерживаться структуры всех гайдов и писать полноценные разделы `Обзор задачи` и `Требования`. Гайд достаточно крупный хотелось бы иметь понимание, что будет разбираться/какие знания я получу после прочтения и т.д. Понимая это, пользователь должен иметь возможность перескочить часть разделов.
>
> По разделам хочется высокоуровневого описания без деталей, которые описываются в самих разделах. 
>
> В разделе `Обзор задачи` также хотелось бы получить ссылку на репозиторий с исходниками и с комментариями типа: 
> - проект, который взят в качестве примера находится тут 
> - master ветка соответствует изначальному состоянию
> - в остальных ветках репозитория можно найти готовые решения, которые будут пошагово рассмотрены в гайде. 
...
> **REVIEW**
>
> Подготовка приложения -> Обзор задачи
>
> Это похоже на ТЗ, часть раздела Обзор задачи.
...
> **REVIEW**
>
> Подготовка приложения -> Требования
>
> Основной посыл раздела Требования: что мне нужно сделать, чтобы перейти к разработке и отладке конфигураций. Желательно, чтобы вся инсталляция была в нём без куском раскиданных по гайду.
...
> **REVIEW**
>
> Не хватает контакта с пользователем, чётких шагов воспроизведения, которые должны выделяться из общего текста:
>
> - Склонируем Git-репозиторий проекта 
>   ```shell
>   git clone ... 
>   ```
>
> - Cоздадим файл werf.yaml
> - Добавим секцую X
> - ... 
>
> Много информации, сложно удерживать нить повествования: заголовки + оформление 
...
> **REVIEW**
>
> Как такового репозитория в образе не создаётся, директория .git отсутствует. На стадии git archive все исходники пакуются в архив (с учётом всех параметров) и распаковываются внутри контейнера. На остальных стадиях применяются патчи с разницей.
...
> **REVIEW**
>
> Тут скорее про то, что при использовании директивы `git`, werf использует репозиторий, в котором расположен `werf.yaml`, как базовый.
...
> **REVIEW**
>
> При работе с удалёнными git репозиториями не рекомендуется использовать ветку (через параметр branch или с master веткой, которая по умолчанию. В остальном никаких ограничений мы не накладываем.
...
> **REVIEW**
>
> Тут можно добавить и про интроспект, как инструмент отладки собранных стадий, а не только тех, что упали
...
> **REVIEW**
>
> Очень много примитивов было разобрано. Как вариант делать сводку в начале или в конце каждого раздела.
> 
> В разделе были/будут разобраны следующие разделы, более подробно смотрите в соответствующих разделах документации:
> - ...
> - ...
> - ...
...
> **REVIEW**
>
> Про ci-env можно вынести в отдельный раздел 
...
> **REVIEW**
>
> Практически все опции werf можно использовать с переменными окружения. Команда ci-env проставляет предопределенные значения опций, которые будут использоваться всеми командами werf в shell-сессии, на основе переменных окружений CI.    
...
> **REVIEW**
>
> Сейчас стадии полностью изолированы: стадия используются только лишь в том случае, если коммит используемый при сборке этой стадии является родительским по отношению к текущему коммиту. Таким образом, стадии двух веток, которые ответвились от master, никогда не пересекаются.    
